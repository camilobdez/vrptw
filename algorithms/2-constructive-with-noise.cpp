...
double generateRandomNoise(double noiseRange, double dist, default_random_engine& generator) {
    ...
    for (int i=0; i<n; i++) {
        ...
        for (int j=0; j<n; j++) {
            ...
            if ( (sal + dist <= timew[j].se && sal + dist + s[j]+d[0][j]<=timew[0].se && Q-q[i]-q[j]>=0) || j==0) {
                if (i==0) {
                    double noise = generateRandomNoise(coef, dist, generator);
                    adj[i].pb(make_pair(Beta * dist - (1-Beta) * timew[j].se + noise, j));
                } else {
                    double noise = generateRandomNoise(coef, dist, generator);
                    if ((instance=="1" || instance=="2" || instance=="7" || instance=="13" || instance=="14") && dist>d[j][0]) continue;
                    adj[i].pb(make_pair(
                        alpha * dist + (1-alpha) * timew[j].fi  + noise
                         ,j));
                }
            }
        }
