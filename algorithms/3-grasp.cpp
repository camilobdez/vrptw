...
int choose_candidate(default_random_engine& generator) {
    geometric_distribution<> geo_d(p);
    return geo_d(generator);
}

void GRASP(double current_time, double current_dist, int capacity, int current_node, default_random_engine& generator) {
    ...
    vector<pair<double, int>> candidate_list;
    ...
    for (auto &[ _, neighbor]: adj[current_node]) {
        ...
        if ( (current_time + dist <= timew[neighbor].se && capacity >= q[neighbor]) &&
            ...
            to_zero = true;
            candidate_list.pb({_, neighbor});
        }
    }

    if (!to_zero) {
        GRASP(current_time + d[0][current_node], current_dist + d[0][current_node], capacity,0, generator);
    } else {
        int chosen = choose_candidate(generator);

        while (chosen >= sz(candidate_list)) chosen = choose_candidate(generator);

        int neighbor = candidate_list[chosen].se;
        double dist = d[current_node][neighbor];

        GRASP(current_time + dist, current_dist + dist, capacity-q[neighbor], neighbor, generator);
    ...
    for (int i=0; i<n; i++) {
        ...
        for (int j=0; j<n; j++) {
            ...
            if ( (sal + dist <= timew[j].se && sal + dist + s[j]+d[0][j]<=timew[0].se && Q-q[i]-q[j]>=0) || j==0) {
                if (i==0) {
                    adj[i].pb(make_pair(Beta * dist - (1-Beta) * timew[j].se + coef, j));
                } else {
