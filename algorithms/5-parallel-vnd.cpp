            ...
            for (int j = 0; j <= n; j++) {
                ...
                if (j == 0 || (earliest + d[i][j] <= nodes[j].l && earliest + d[i][j] + nodes[j].s + d[j][0] <= nodes[0].l && nodes[i].q + nodes[j].q <= Q)) {
                    ...
                    if (i == 0) {
                        ...
                        adj[i].pb(make_pair(alpha * d[i][j] + (1 - alpha) * nodes[j].e - noise * 1e3, j));
                    }
                }
            }
            sort(all(adj[i]));
...
Solution perturb_solution(Solution& current_solution, int opt, int thread_id) {
    unsigned seed = chrono::system_clock::now().time_since_epoch().count() + thread_id;
    mt19937 gen(seed);

    Solution new_solution = current_solution;

    if (opt == 1) new_solution = perturb_solution_1(new_solution, gen);
    else if (opt == 2) new_solution = perturb_solution_2(new_solution, gen);
    else if (opt == 3) new_solution = perturb_solution_3(new_solution, gen);
    else if (opt == 4) new_solution = perturb_solution_4(new_solution, gen);
    else new_solution = perturb_solution_5(new_solution, gen);

    return new_solution;
}

Solution first_local_search(Solution initial_sol, int opt) {
    ...
    while (improved) {
        ...
        for (int i = 0; i < best_solution.num_vehicles && !improved; i++) {
            Vehicle& v1 = best_solution.vehicles[i];

            if (opt == 5) {
                Solution new_solution = two_opt(best_solution, v1);
                if (compute_cost_2(new_solution) < compute_cost_2(best_solution)) {
                    best_solution = new_solution;
                    improved = true;
                }
            }

            for (int j = 0; j < best_solution.num_vehicles && !improved && (opt != 3 && opt != 5); j++) {
                if (i == j) continue;
                Vehicle& v2 = best_solution.vehicles[j];

                // Apply shift operation
                for (int idx1 = 0; idx1 < sz(v1.route) - 1 && !improved && (opt==1); idx1++) {
                    for (int idx2 = 1; idx2 < sz(v2.route) - 1  && !improved; idx2++) {
                        Solution new_solution = shift_op(best_solution, v1, v2, idx1, idx2);
                        if (compute_cost_2(new_solution) < compute_cost_2(best_solution)) {
                            best_solution = new_solution;
                            improved = true;
                        }
                ...
                for (int idx1 = 0; idx1 < sz(v1.route) - 1  && !improved && (opt==2); idx1++) {
                    for (int idx2 = 0; idx2 < sz(v2.route) - 1  && !improved; idx2++) {
                        Solution new_solution = exchange_op(best_solution, v1, v2, idx1, idx2);
                        if (compute_cost_2(new_solution) < compute_cost_2(best_solution)) {
                            best_solution = new_solution;
                            improved = true;
                        }
                ...
                for (int idx1 = 1; idx1 < sz(v1.route) - 1  && !improved && (opt==4); idx1++) {
                    for (int idx2 = 1; idx2 < sz(v2.route) - 1  && !improved; idx2++) {
                        Solution new_solution = SWAP(best_solution, v1, v2, idx1, idx2);
                        if (compute_cost_2(new_solution) < compute_cost_2(best_solution)) {
                            best_solution = new_solution;
                            improved = true;
                        }
            ...
            // Apply rearrange operation within the same vehicle (intra-route optimization)
            for (int start_idx = 1; start_idx < sz(v1.route) - 1 && !improved && opt==3; start_idx++) {
                for (int end_idx = start_idx; end_idx < sz(v1.route) - 1  && !improved; end_idx++) {
                    for (int insert_position = 1; insert_position < sz(v1.route)  && !improved; insert_position++) {
                        if (insert_position >= start_idx && insert_position <= end_idx) continue;
                        Solution new_solution = rearrange_op(best_solution, v1, start_idx, end_idx, insert_position);
                        if (compute_cost_2(new_solution) < compute_cost_2(best_solution)) {
                            best_solution = new_solution;
                            improved = true;
                        }
...
Solution parallel_VNS(int num_threads, int time_limit) {
    auto start_time = chrono::system_clock::now();

    vector<Solution> local_best(num_threads);
    bool go = true;
    ...
    #pragma omp parallel num_threads(num_threads)
    {
        ...
        //#pragma omp critical
        //cout << "Initial solution for thread " << thread_id << " with cost: " << local_best[thread_id].num_vehicles << " and temperature: " << local_best[thread_id].total_distance << endl;
    }

    Solution global_best = *min_element(local_best.begin(), local_best.end(), [](const Solution& a, const Solution& b) {return compute_cost_2(a) < compute_cost_2(b);});

    while (go) {
        #pragma omp parallel num_threads(num_threads) shared(global_best, local_best, go)
        {
            int thread_id = omp_get_thread_num();
            int j = 1;

            while (j <= 5) {
                Solution new_solution = perturb_solution(local_best[thread_id], j, thread_id);
                new_solution = first_local_search(new_solution, j);
                if (compute_cost_2(new_solution) < compute_cost_2(local_best[thread_id])) {
                    #pragma omp critical
                    {
                        local_best[thread_id] = new_solution;
                        //cout << "Thread " << thread_id << ", operation " << j << " updated its solution with new cost: " << local_best[thread_id].num_vehicles << " " << local_best[thread_id].total_distance << endl;
                    }
                    j = 1;
                } else {
                    j++;
                ...
                if (elapsed_seconds.count()+2 >= time_limit) {
                    go = false;
                    break;
                }
            }
            ...
            #pragma omp single
            {
                ...
                if (compute_cost_2(best_local_solution) < compute_cost_2(global_best)) {
                    ...
                    //cout << "Global best updated with new cost: " << global_best.num_vehicles << " " << global_best.total_distance << endl;
                }
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    int num_threads = 10;
    Solution best_solution = parallel_VNS(num_threads, time_limit);
